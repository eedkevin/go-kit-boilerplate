...
func MakeHTTPHandler(endpoints Endpoints, options ...httptransport.ServerOption) http.Handler {
	...
	m.Methods("POST").Path("/distance").Handler(httptransport.NewServer(
		endpoints.DistanceEndpoint,
		DecodeHTTPDistanceZeroRequest,
		EncodeHTTPGenericResponse,
		serverOptions...,
	))
	return m
...
// DecodeHTTPDistanceZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded distance request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPDistanceZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
	var req pb.DistanceRequest
	buf, err := ioutil.ReadAll(r.Body)
	if err != nil {
		return nil, errors.Wrapf(err, "cannot read body of http request")
	}
