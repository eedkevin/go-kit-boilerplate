...
func MakeHTTPHandler(endpoints Endpoints, options ...httptransport.ServerOption) http.Handler {
	...
	m.Methods("POST").Path("/place_order").Handler(httptransport.NewServer(
		endpoints.PlaceOrderEndpoint,
		DecodeHTTPPlaceOrderZeroRequest,
		EncodeHTTPGenericResponse,
		serverOptions...,
	))
	return m
...
// DecodeHTTPPlaceOrderZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded placeorder request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPPlaceOrderZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	defer r.Body.Close()
