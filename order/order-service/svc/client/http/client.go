...
// Package http provides an HTTP client for the Order service.
package http
...
func New(instance string, options ...httptransport.ClientOption) (pb.OrderServer, error) {
	...
	var PlaceOrderZeroEndpoint endpoint.Endpoint
	{
		PlaceOrderZeroEndpoint = httptransport.NewClient(
			"POST",
			copyURL(u, "/place_order"),
			EncodeHTTPPlaceOrderZeroRequest,
			DecodeHTTPPlaceOrderResponse,
			options...,
		).Endpoint()
	}
	...
	return svc.Endpoints{
		...
		PlaceOrderEndpoint: PlaceOrderZeroEndpoint,
	}, nil
}
...
// DecodeHTTPPlaceOrderResponse is a transport/http.DecodeResponseFunc that decodes
// a JSON-encoded PlaceOrderResponse response from the HTTP response body.
// If the response has a non-200 status code, we will interpret that as an
// error and attempt to decode the specific error message from the response
// body. Primarily useful in a client.
func DecodeHTTPPlaceOrderResponse(_ context.Context, r *http.Response) (interface{}, error) {
	defer r.Body.Close()
	buf, err := ioutil.ReadAll(r.Body)
	if err == io.EOF {
		return nil, errors.New("response http body empty")
...
// EncodeHTTPPlaceOrderZeroRequest is a transport/http.EncodeRequestFunc
// that encodes a placeorder request into the various portions of
// the http request (path, query, and body).
func EncodeHTTPPlaceOrderZeroRequest(_ context.Context, r *http.Request, request interface{}) error {
	strval := ""
	_ = strval
	...
	toRet := request.(*pb.PlaceOrderRequest)
